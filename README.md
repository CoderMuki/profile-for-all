...
- Build your profile and create a website and host it.
...
After the site is built to your satisfaction in local environment follow the below sites.

- Run `ng build` in your project root folder `dist` folder containing your compiled project will be created.
...
- Add the `firebaseConfig` interface in your `app.component.ts` file and uncomment the `const app` and `const analytics` lines.
- Run `ng build` to update all codes. 
- In the project root folder run `firebase init`.
- User arrows to move down and select `Hosting` using spacebar.
- Select `Use an existing project` and select your project.
- For public directory enter `dist` and configure as a Single Page App to `Y`.
- Set automatic builds to `N` when prompted.
- Firebase initialization is completed and a `firebase.json` file will be created outside of `src` folder.
